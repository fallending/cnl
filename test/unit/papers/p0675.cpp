...
namespace acme {
    ...
    template<class Rep>
    struct smart_integer {
        explicit constexpr smart_integer(Rep const& rhs)
            : _value(rhs)
        {
        }
            ...
            smart_integer<LhsRep> const& lhs, smart_integer<RhsRep> const& rhs)
    {
        ...
        if constexpr (
                std::numeric_limits<LhsRep>::is_signed == std::numeric_limits<RhsRep>::is_signed) {
...
namespace {
    ...
    {  // NOLINT
        // use an unsigned 16-bit integer to approximate a real number with 2 integer and 14
        // fraction digits
    ...
    namespace desirata {
        ...
        using set_num_digits_t = cnl::set_digits_t<T, B>;
